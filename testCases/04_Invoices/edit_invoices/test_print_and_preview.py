...
class TestEditInvoice:
    ...
    def test_print_and_preview(self, setup):
        self.driver = setup
        self.driver.get(self.base_url)
        self.login = LoginPage(self.driver)
        self.login.login_to_application(self.username, self.password)
        sleep(MEDIUM_WAIT)
        self.print_and_preview_invoice_page = InvoicesPage(self.driver)
        sleep(SHORT_WAIT)
        self.print_and_preview_invoice_page.invoice_action_print_and_preview()
        sleep(SHORT_WAIT)
        self.driver.close()
